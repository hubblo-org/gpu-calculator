...
<table>
  <caption>Functional unit results</caption><thead
    ><tr
  ...
  ><tbody
    >{#each results as row}<tr
        ><td>{row.scope}</td><td>{row.amount}</td><td>{row.unit}</td
        >{#each Object.values(row.impacts) as impact}<td>{impact}</td>{/each}</tr
      >{/each}</tbody
  >
</table>
